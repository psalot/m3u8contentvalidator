  ...
  module Util
    ...
    def run(command, *args)
      ...
      if status.success?
        ...
        my_object={"error"=>"Error running #{command} #{args.inspect}:\n\n#{error_str}"}
  ...
  class Playlist < Resource
    ...
    def parse_urls
      ...
      if playlist_urls.size == 0 && fragment_urls.size == 0
        my_object = { "status" => "400", "message"=>"No URLs found in playlist" }
          Array1 << JSON[JSON.pretty_generate(my_object)]
      end
